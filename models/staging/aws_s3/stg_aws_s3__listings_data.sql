with source as (
  ...
  renamed as (
      select
        ...
        {{ adapter.quote("HOST_VERIFICATIONS") }}::array AS host_verification_methods_array,
        ...
        COALESCE({{ adapter.quote("BEDROOMS") }},1) AS no_of_bedrooms,
        COALESCE({{ adapter.quote("BEDS") }},1) AS no_of_beds,
        ...
        regexp_replace({{ adapter.quote("PRICE") }}, '[$,]', '')::float AS listing_price_usd,
