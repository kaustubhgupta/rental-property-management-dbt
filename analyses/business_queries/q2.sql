...
-- listing price on or just before 2021-07-12
...
-- listing price on or just before 2022-07-11
...
-- listing-neighborhood mapping
listings_neighborhood as (
  select distinct listing_id, listing_neighborhood
  from {{ ref('listings_daily_report') }}
),

-- listing 2021 and 2022 price combined with price difference calculation
per_listing_diff as (
  select
    l.listing_id,
    ...
    p21.price_2021,
    p22.price_2022,
    coalesce(p22.price_2022,0) - coalesce(p21.price_2021,0) as price_diff
...
/*
final selection on listing_neighborhood level with avg price increase change
only selecting records where price diff is positive
*/
select
